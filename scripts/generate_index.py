...
def scan_ctfs(base_dir="."):
    ...
    for ctf_name in os.listdir(base_dir):
        ...
        if not os.path.isdir(ctf_path) or ctf_name in ["assets", "scripts", ".git"]:
            continue
        ...
        ctf_data = {
            "id": ctf_id,
            "name": ctf_name,
            "status": "ended",  # you can adjust manually later
            "banner": f"assets/{ctf_id}.png",  # put a placeholder image if missing
            "categories": {}
        }
